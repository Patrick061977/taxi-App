...
// ═══════════════════════════════════════════════════════════════
// 🆕 v6.25.5: VERIFIZIERUNGSCODE — SMS oder WhatsApp
// Für buchen.html (öffentliche Buchungsseite)
// ═══════════════════════════════════════════════════════════════
...
exports.sendVerificationCode = onRequest(
    ...
    async (req, res) => {
        ...
        const { phone, method } = req.body; // method: 'whatsapp' oder 'sms'
        if (!phone) return res.status(400).json({ error: 'Telefonnummer fehlt' });

        // Telefonnummer formatieren
        let formatted = String(phone).replace(/[\s\-\/\(\)]/g, '');
        if (formatted.startsWith('0') && !formatted.startsWith('00')) formatted = '+49' + formatted.slice(1);
        if (!formatted.startsWith('+')) formatted = '+' + formatted;

        // 4-stelligen Code generieren
        const code = String(Math.floor(1000 + Math.random() * 9000));

        // Code in Firebase speichern (10 Min gültig)
        const codeRef = db.ref('verificationCodes/' + formatted.replace('+', ''));
        await codeRef.set({
            code,
            phone: formatted,
        ...
        const message = `Ihr Buchungscode: ${code}\n\nUmwelt Taxi Usedom\n038378 22022`;

        try {
            if (method === 'whatsapp') {
                // WhatsApp — kostenlos über Business API
                const waResult = await sendWhatsAppMessage(formatted, message);
                if (waResult) {
                    console.log(`✅ Verifizierungscode per WhatsApp an ${formatted}`);
                    return res.status(200).json({ success: true, method: 'whatsapp' });
                }
                // WhatsApp fehlgeschlagen → Fallback auf SMS
            ...
            console.error('❌ Verifizierungscode Fehler:', error.message);
...
exports.verifyCode = onRequest(
    ...
    async (req, res) => {
        ...
        if (!phone || !code) return res.status(400).json({ error: 'Telefonnummer und Code erforderlich' });
        ...
        if (!snap.exists()) return res.status(400).json({ error: 'Kein Code für diese Nummer', valid: false });

        const data = snap.val();
        if (Date.now() > data.expiresAt) {
            await codeRef.remove();
            return res.status(400).json({ error: 'Code abgelaufen — bitte neu anfordern', valid: false });
        }
        if (data.code !== String(code).trim()) {
            return res.status(400).json({ error: 'Falscher Code', valid: false });
        }

        // Code als verifiziert markieren + löschen
        await codeRef.update({ verified: true, verifiedAt: Date.now() });
        setTimeout(() => codeRef.remove(), 60000); // Nach 1 Min aufräumen

        return res.status(200).json({ valid: true, phone: formatted });
    }
);
