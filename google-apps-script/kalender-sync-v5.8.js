...
// Version: 5.8 - Zwischenstopp-Komplettadresse im Titel + Suffix-Cleanup "Kaiserbäder" raus.
//                Patrick (07.09. 13:16 Bridge): "beim Google-Script wird der Zwischenstopp abgeschnitten"
//                + "ich brauche Ahlbeck, Bansin und Heringsdorf. Ich brauche keine Kaiserbäder dahinter".
//                Fixes:
//                  - shortPart nimmt jetzt Name + Straße (nicht nur POI-Name vor 1. Komma)
//                  - cleanCity(addr): entfernt "-Kaiserbäder" / "Kaiserbaeder" Suffix
//                  - Wird auf pickup, destination und wpShort angewendet
// Version: 5.7 - Pauschalpreis-Prefix
// Version: 5.5 - 🔐 Firebase Anonymous Auth (Phase A DSGVO Rules forderten auth!=null fuer /rides + /settings).
//                Patrick (14.06.): 'das google script funktioniert nicht'. Befund: UrlFetchApp ohne Auth → 401 Permission Denied.
//                Fix: getAnonAuthToken() generiert + cached Anon-ID-Token (1h gueltig, Refresh 50 Min).
//                Beide Firebase-Reads (fetchRidesFromFirebase, loadExportSettings) bekommen ?auth=TOKEN.
// Version: 5.4 - 🏷️ Pauschalpreis-Marker (Titel + Beschreibung). Patrick (03.05.): 'Wie wird das nachher uebernommen, wird das als Pauschalpreis angezeigt im Kalender'. Erkennung: ride.priceFixed=true ODER notes enthaelt 'Pauschalpreis 36€...'.
...
const CONFIG = {
  ...
  // 🔐 v5.5: Firebase Web API Key (Anonymous Auth) — gleicher Key wie Web-App
  FIREBASE_WEB_API_KEY: 'AIzaSyD2eHFuplImxBCijd3MWlKyCwXUZHLmhhE'
};
// ═══════════════════════════════════════════════════════════════
// 🔐 v5.5: ANONYMOUS AUTH TOKEN HOLEN (gecached)
// ═══════════════════════════════════════════════════════════════
const ANON_TOKEN_KEY = 'firebaseAnonIdToken';
const ANON_TOKEN_EXP_KEY = 'firebaseAnonIdTokenExp';
function getAnonAuthToken() {
  try {
    const props = PropertiesService.getScriptProperties();
    const cached = props.getProperty(ANON_TOKEN_KEY);
    const cachedExp = props.getProperty(ANON_TOKEN_EXP_KEY);
    if (cached && cachedExp && Date.now() < parseInt(cachedExp)) {
      return cached;
    }
    const url = 'https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=' + CONFIG.FIREBASE_WEB_API_KEY;
    ...
    if (code !== 200) {
      console.error('❌ Anon-Auth fehlgeschlagen Code ' + code + ': ' + resp.getContentText().substring(0, 200));
      return null;
    }
    const data = JSON.parse(resp.getContentText());
    if (!data.idToken) {
      console.error('❌ Anon-Auth: kein idToken in Response');
      return null;
    }
    // Token gilt 1h, wir cachen 50 Min sicherheitshalber
    props.setProperty(ANON_TOKEN_KEY, data.idToken);
    props.setProperty(ANON_TOKEN_EXP_KEY, String(Date.now() + 50 * 60 * 1000));
    console.log('🔐 Neuer Anon-Auth-Token generiert (gueltig 50 Min)');
    return data.idToken;
  } catch (e) {
    console.error('❌ getAnonAuthToken-Exception:', e);
    return null;
  }
}
function firebaseUrl(path) {
  const token = getAnonAuthToken();
  const sep = path.indexOf('?') >= 0 ? '&' : '?';
  return CONFIG.FIREBASE_URL + path + (token ? sep + 'auth=' + token : '');
}
...
function loadExportSettings() {
  try {
    // v5.5: Anon-Auth Token an URL anhaengen (Phase-A-Rules: auth!=null)
    const url = firebaseUrl('/settings/calendarExport.json');
...
function fetchRidesFromFirebase() {
  // v5.5: Anon-Auth Token (Phase-A-Rules: /rides .read=auth!=null)
  const url = firebaseUrl('/rides.json');
...
function createOrUpdateCalendarEvent(calendar, ride) {
  try {
    ...
    // v5.8: Waypoint komplett schreiben (Name + Strasse, nicht nur POI) + Kaiserbaeder-Cleanup
    function cleanCity(addr) {
      if (!addr) return addr;
      return String(addr)
        .replace(/-Kaiserb(ä|ae)der/gi, '')  // "Ahlbeck-Kaiserbäder" → "Ahlbeck"
        .replace(/\s*Kaiserb(ä|ae)der\b/gi, ''); // "17419 Ahlbeck Kaiserbäder" → "17419 Ahlbeck"
    }
    function wpDisplay(addr) {
      // Nimm die ersten 2 Kommagetrennten Teile (Name + Strasse) statt nur POI-Namen
      if (!addr) return '';
      var parts = String(addr).split(',').map(function(p){ return p.trim(); }).filter(Boolean);
      var text = parts.length >= 2 ? parts[0] + ', ' + parts[1] : parts[0];
    ...
    if (ride.waypoints && ride.waypoints.length > 0) {
      ...
      var wpShort = wpNames.map(function(w) {
        ...
        var shortPart = wpDisplay(addr);
        var paxName = (typeof w === 'object') ? (w.name || '') : '';
        if (paxName) shortPart += ' (' + paxName + ')';
        return shortPart || cleanCity(addr);
      });
      titleParts.push(`${_flatPrefix}🚕 ${cleanCity(ride.pickup) || 'Unbekannt'} → 🔶${wpShort.join(' → 🔶')} → ${cleanCity(ride.destination) || 'Unbekannt'}`);
    } else {
      titleParts.push(`${_flatPrefix}🚕 ${cleanCity(ride.pickup) || 'Unbekannt'} → ${cleanCity(ride.destination) || 'Unbekannt'}`);
    }
...
function createEventDescription(ride) {
  ...
  lines.push('');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  lines.push('📝 Erstellt von: CalendarSync v5.4 (🏷️ Pauschalpreis-Marker)');
...
function findExistingEvent(calendar, firebaseId) {
  ...
  for (const event of events) {
    ...
    // v5.7 (Patrick 15.07.): startsWith('🚕') scheiterte bei Pauschalpreis-Fahrten (Titel = '🏷️ 🚕 ...').
    //   Deshalb wurden Events nie gefunden → alle 3-5 Min DUPLIKAT angelegt (Ivonne Koenig 15.07.
    //   Anna Ostsee → Świnoujście). Fix: includes statt startsWith — verkraftet 🏷️-Prefix.
...
function removeOldEvents(calendar, currentRides) {
  ...
  events.forEach(event => {
    const title = event.getTitle();
    // v5.7: includes statt startsWith (Pauschalpreis-Titel starten mit 🏷️)
