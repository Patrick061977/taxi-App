...
public class MapPickerActivity extends AppCompatActivity {
    ...
    private String buildMapHtml(String initialQuery) {
        ...
        return "<!DOCTYPE html><html><head><meta charset='utf-8'>"
            ...
            // v6.62.253: addressdetails=1 fuer kompakte Adresse statt langem display_name.
            // Patrick (11:07): '22, Am Schulberg, Neuhof, Heringsdorf, Vorpommern-Greifswald,
            // Mecklenburg-Vorpommern, 17424, Deutschland' -> kompakter:
            // 'Am Schulberg 22, 17424 Heringsdorf-Neuhof'
            + " var url='https://nominatim.openstreetmap.org/search?format=json&limit=1&countrycodes=de&addressdetails=1&q='+encodeURIComponent(q);"
            + " fetch(url,{headers:{'User-Agent':'TaxiHeringsdorf/6.62.253'}})"
            + "  .then(function(r){return r.json();})"
            + "  .then(function(arr){"
            ...
            + "    if(road){var s=road;if(nr)s+=' '+nr;parts.push(s);}"
            + "    var cityCombined=city;if(suburb && suburb!==city)cityCombined=city?(city+'-'+suburb):suburb;"
            + "    if(pc||cityCombined){var s='';if(pc)s+=pc;if(cityCombined)s+=(s?' ':'')+cityCombined;parts.push(s);}"
            + "    var addr=parts.length?parts.join(', '):hit.display_name||'';"
