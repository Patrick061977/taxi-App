...
// Version: 3.8 - NUR GEÄNDERTE TERMINE! - Build 20260119-1735
// 🆕 NEU: Prüft updatedAt - aktualisiert nur was sich geändert hat!
// 🆕 NEU: Viel effizienter - weniger API-Calls!
// 🆕 NEU: Sync-Timestamp in Properties gespeichert!
// 🆕 v3.6: Unterstützt customerMobile + customerPhone getrennt!
// ═══════════════════════════════════════════════════════════════
// 🔧 KONFIGURATION
// ═══════════════════════════════════════════════════════════════
const CONFIG = {
...
function syncFirebaseToCalendar() {
  console.log('🚀 Starte SMARTE Kalender-Synchronisation v3.8...');
  ...
  try {
    ...
    // Nur aktive Stati behalten
    const activeChangedRides = changedRides.filter(ride => {
      const activeStatuses = ['vorbestellt', 'new', 'accepted', 'assigned', 'pending', 'confirmed'];
...
function createEventDescription(ride) {
  ...
  lines.push('');
  lines.push('━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  lines.push('📝 Erstellt von: CalendarSync v3.8 (Smart Sync)');
...
function removeOldEvents(calendar, currentRides) {
  ...
  const cancelledIds = new Set(
    currentRides
      .filter(r => ['storniert', 'cancelled'].includes(r.status))
      .map(r => r.firebaseId)
  );
  ...
  events.forEach(event => {
    ...
    if (!firebaseId) {
      ...
      if (cancelledIds.has(extractedId) || !currentIds.has(extractedId)) {
        shouldDelete = true;
      }
    } else {
      if (cancelledIds.has(firebaseId) || !currentIds.has(firebaseId)) {
        shouldDelete = true;
      }
    }
    ...
    if (shouldDelete) {
      event.deleteEvent();
      removed++;
      console.log('🗑️ Entfernt:', title.substring(0, 50) + '...');
    }
  });
...
function setupAutomaticSync() {
  ...
  console.log('✅ Neuer Smart-Sync Trigger erstellt: alle ' + CONFIG.SYNC_INTERVAL + ' Min');
}
// ═══════════════════════════════════════════════════════════════
// 🧪 TEST-FUNKTION
// ═══════════════════════════════════════════════════════════════
function testSync() {
  console.log('🧪 TEST-MODUS v3.8 - SMART SYNC');
  console.log('═══════════════════════════════════════════');
  console.log('🆕 Prüft updatedAt - nur Änderungen werden synced!');
  console.log('🆕 Timestamp in Properties gespeichert');
  console.log('🆕 Viel effizienter - weniger API-Calls');
  console.log('');
